...
@inline function Ct_turbine(U, turb)
    ...
    R = promote_type(typeof(U), typeof(Ct_rated))

    if U <= 0
        return zero(R)

    elseif U < u_in
        return R(Ct_rated * exp(5 * (U / u_in - 1)))
    ...
    elseif U < u_out
        ...
        return R(Ct_from_Cp_cardano(Cp))
    else
        return zero(R)
    end
end
...
@inline function turbine_power_density_single(u, x, equations, turb;
    ...
    R = promote_type(eltype(u), typeof(dA))
    zeroR = zero(R)
    ...
    Cp = Cp_turbine(U, turb)
    return R(0.5) * rho * Cp * R(turb.At) * dA * U^3
end
