...
# tidal values
T = 0.25 * 60 * 60 * sqrt(g / D) # hr * min/hr * sec/min * sqrt(m/s^2 * 1/m)
ω = 2π / T
Aₜ = 4 * 0.275 / D
...
base = (; mesh, solver, equations, initial_condition, boundary_condition,
        friction_source, sponge_source,
        u_rated, u_in, u_out, h_min,
        t_out, Δt, tspan,
)

function build_semi(p_list, base)
    # (3) make turbines
    turbines = [Turbine(; x0 = p[1], y0 = p[2], base.u_rated, base.u_in, base._out, base.h_min) for p in p_list]
    turbine_source = TurbineFriction(turbines)
    source_terms = CombinedSource((base.sponge_source, base.friction_source, turbine_source))
...
function objective(p, base; tspan, saveat = 0.05, rho = 1000.0)
    ...
    # (7) output setput
    analysis_callback = AnalysisCallback(semi, interval = base.t_out)
